...
/**
 * Get the appropriate icon for an OAuth provider.
 */
function getProviderIcon(name: string, className: string) {
  switch (name.toLowerCase()) {
    ...
    default:
      return <OAuthIcon className={className} />;
  }
}
                ...
                <div className="space-y-3">
                  {config.oauth_providers.map((provider) => (
                    <a
                      ...
                      className={cn(
                        "flex w-full items-center justify-center gap-3 rounded-[var(--radius-md)]",
                        "border border-[var(--color-border)] bg-[var(--color-card-alt)]",
                        "px-4 py-2.5 text-sm font-medium text-[var(--color-foreground)]",
                        "transition-colors duration-150",
                        "hover:bg-[var(--color-hover)] hover:border-[var(--color-border-active)]",
                        "focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]",
                        "focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-card)]",
                      )}
                    >
                      {getProviderIcon(provider.name, "h-5 w-5")}
